...
import { AuthProvider, User, UserRole, UserStatus } from '@pulse/database';
...
export class AuthController {
  ...
  private writeRefreshCookie(response: Response, token: string) {
    response.cookie('pulse_refresh', token, {
      httpOnly: true,
      sameSite: 'lax',
  ...
  async refresh(
    ...
    const result = await this.auth.refresh(
      request.cookies?.pulse_refresh ?? parsed.refreshToken,
  ...
  async logout(
    ...
    await this.auth.logout(request.cookies?.pulse_refresh);
    response.clearCookie('pulse_refresh', { path: '/api/auth' });
    return { ok: true };
  }
